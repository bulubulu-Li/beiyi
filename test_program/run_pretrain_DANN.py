...
def train_once_DANN(model,optimizer,optimizer2,train_iterator,train_identity_iterator,len_train_iter,params,epoch_i):
    #print(f'-- epoch is {epoch_i}, cnn model train begin -- ')
    model.train()
    train_loss = 0.0
    label_list = []
    ...
    for i in range(len_train_iter):
        ...
        loss2=loss_funct_identity(predict_y_iden,batch_y_iden)
...
def run_train(model, optimizer,use_eval=1):
    ...
    for i in range(EPOCHS):
        ...
        len_dataloader = min(train_len, train_identity_len)
        accuracy,precision,recall,specificity,train_auc, train_loss, tmp_model = train_once_DANN(model, optimizer, optimizer2,train_iterator,train_identity_iterator,len_dataloader,params,i)
        ...
        if use_eval:
            ...
            if all_score > best_score:
                model_path = os.path.join(model_root_path,f'pretrain_epoch{i}_{start_time_tag}.pkl')
