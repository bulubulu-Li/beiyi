...
def get_train_data(name_list,flag):
    ...
    this_root_path = os.path.join(f'./bei_yi_70/0602_single/{flag}_5')
...
class  CNN_INPUT_cross_fine_tune():
    def __init__(self, config):
        ...
        self.use_pretrain = 0 #config.use_pretrain
        ...
        self.train_name_list = ['陈康霖', '郑桓宇', '白靖宇', '王震林', '谢景恬', 
                        '张念晨', '冯雨欣', '王一回', '吕雅涵', '卢一赫', 
                        '孙嘉豪', '陈宇轩', '卢婧之', '宋钰苇', '耿子卓', '孙奥发', '黄嘉琪', '陈柳妍'
                        ]
        # self.test_name_list = [ '柳叶恩', ]
        # self.test_name_list = [ 'mit', ]
        # self.test_name_list = [ '测试病', ]
        self.test_name_list = [ '谢景恬', ]
        
        self.new_test_name_list = [ '啦啦啦啦', ]
        self.new_test_name_list = [ '谢景恬', ]
        self.new_test_name_list = [ '柳叶恩', ]
        self.new_test_name_list = [ '耿子卓', ]
        self.new_test_name_list = [ '柳叶恩','谢景恬' ]
        # self.test_name_list = [ '测试病人', ]
        # self.test_name_list = [ '新病人', ]
        self.test_num = len(self.new_test_name_list)
    ...
    def load_test_data(self, patient_id=0):
        ...
        print(f'test_name_list is {test_name_list} ')
        pos_test_data = get_train_data(test_name_list, 'pos')#103
        neg_test_data = get_train_data(test_name_list, 'neg')#750

        all_scale_way_list = ['add','sub','add_sub','mutiply','random'] * 5
        scale_way_list = all_scale_way_list[:self.pos_train_scale]

        pos_fine_tune_data, pos_test_data = get_pretrain_data(pos_test_data, pretrain_size=self.use_pretrain, flag='pos')#分成两份
        # if self.pos_train_scale > 0:
        #     print(f'use scale is {self.pos_train_scale}, scale_way_list is {scale_way_list} ')
    ...
    def load_test_data_all(self, patient_id=0):
        test_name_list = self.new_test_name_list[patient_id:patient_id+1]
        print(f'test_name_list is {test_name_list} ')
        # pos_test_data = get_train_data(test_name_list, 'pos')#103
        neg_test_data = get_train_data(test_name_list, 'neg')#71,250,19
        # pos_fine_tune_data, pos_test_data = get_pretrain_data(pos_test_data, pretrain_size=self.use_pretrain, flag='pos')#分成两份
        # if self.pos_train_scale > 0:
        #     print(f'use scale is {self.pos_train_scale}, scale_way_list is {scale_way_list} ')
        #     pos_fine_tune_data = build_scale_together(pos_fine_tune_data, scale_way_list)
        ...
        test_loader = self.build_data_iterator_all(neg_test_data, 'test')
        # fine_tune_loader = self.build_data_iterator(pos_fine_tune_data, neg_fine_tune_data, 'fine_tune')
        fine_tune_loader = test_loader
        return fine_tune_loader, test_loader

    def build_data_iterator_all(self, neg_arr, flag):
        print(f'------ {flag} iterator begin ')
        # pos_y = np.ones(pos_arr.shape[0])
        neg_y = np.zeros(neg_arr.shape[0])
        ...
        print(f'build {flag} iterator here, neg_num is {neg_arr.shape[0]} ')
